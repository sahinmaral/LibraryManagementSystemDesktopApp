# Kutuphane_Sistemi
C# Windows Form olarak ilk projem
C# üzerinden bir yandan bildiklerimi tekrar edip bir yandan da araştırmalar yapmak için küçük bir kütüphane sistemi yapıyorum.</br>
Veritabanını sorunsuz ve sıkıntısız bir şekilde kullanabilmek için **FilesAboutDatabase içinde sql_server_version.png içindeki** versiyonlarınızın olması , </br>
**FilesAboutDatabase** içinde **.bak uzantılı dosya**yı restore etmeniz,</br>
**SqlAddress.txt dosyasını C:\ içine** attığınız zaman sorunsuz çalışacaktır.
